...
public:
  ...
  QualType VisitType(const Type *T) {
    ...
    // desugaring (e.g.: to check for a pointer/array).
    //
    // FIXME: This is not the right design but we will fix it in later commits.
    //
    // In the case of nested sugar types this means we will run the same
    // "leaf-check" every time we desugar which is potentially wasteful. It
    // could also potentially cause problems because it means if that function
    // emits warnings it has to be done very carefully (no desugaring) to avoid
    // emitting the same warning every time this visitor desguars and calls the
    // leaf check again. This desugaring and re-running the leaf check is
    // basically here to handle the checks in `ValidateBoundsAttrTypeShape` that
    // don't desugar themselves. At the same time we are relying on some of the
    // checks in that function doing their own desugaring so that running the
    // leaf check on sugar passes. This is very fragile.
